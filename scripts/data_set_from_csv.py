...
def remove_keys_with_empty_string_values(missing_value=""):
    def transform_remove_keys_with_empty_string_values(sample:Dict):
        sample_without_missing_value_keys= {}
        for key in sample.keys():
            if len(sample.get(key,missing_value).strip()) > 0:
                sample_without_missing_value_keys[key] = sample[key]
        return sample_without_missing_value_keys
    return transform_remove_keys_with_empty_string_values
